...
//		int pstmt_lno = 0;
		...
		if ((seed.equals("executeQuery") || seed.equals("executeUpdate")|| seed.equals("execute")) && (sliceDirection == 'B' || sliceDirection == 'b')) {
			List<SliceLine> sliceLines1;
			if(Print)System.out.println("Calling do_slice_bw_cg");
			sliceLines1 = SlicerTool.do_slice_bw_cg(cg, builder,appJar, app_folder_className, lno, appSrc, 0, dOptions, cOptions, sstype, seed);
			if (sliceLines1 != null || sliceLines.size() == 0)
				sliceLines = sliceLines1;
...
//===================== next 3 methods are from SourceDirCallGraph.java from WALA-start
	//https://github.com/wala/WALA-start/blob/master/src/main/java/com/ibm/wala/examples/drivers/SourceDirCallGraph.java
	public static CGClass BuildSrcDirCG(String sourceDir, String mainClass)
			throws WalaException, CancelException, IOException {
		// ================ Build CG ============
		long start = System.currentTimeMillis();
...
//		return newCG;
	    ...
	    System.out.println("Time needed to create CG: "+(end - start));
