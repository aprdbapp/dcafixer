		...
		if (patch_is_found) {
			...
			if (QParser.prefered_sol == G.SOLPS || is_ps) {
				...
				if (!QParser.ParserFail && s.get_I().contains('t')) {
					...
					PC.FGPatches++;
					System.out.println("============> WL_QPFailed???");
				}
			}
...
//			if (SignSlice.qp.parserFailed()) {///+++++9/30
	...
	public static List<SliceLine> createBackupSlice(Info info, String srcPath, String slicePath, String contextPath,
		...
		System.out.println("@@ createBackupSlice: callLines : " + callLines + ", queryLines: " + queryLines);
...
//					slice_srclines = slice_srclines + sl.stmt.trim();
	...
	public static List<SliceLine> createBackupSliceConn(Info info, String srcPath, String slicePath, String contextPath,
		...
		System.out.println("@@ createBackupSlice: callLines : " + callLines + ", credintalLines: " + credintalLines + ", allLinesNos:" +sliceCodeLineNos);
//		2- get lines in array list to get code lines
		codeLines = StrUtil.read_lines_list(srcPath);
		//=========== get g  credLines
...
//				}
					if (s.get_V() != '_') {// if not secure
						RplacePHsConn.replacePlaceHolders_and_createTmpVSlices(vSlicesPath, info, true);
						// Find app_md
						if (PatternGenConn.findPatternsAndApply_usingExistingPatchesConn(v_appSrc, v_slicePath,
								G.CONN_TmpVSlicesPath, info, getAppMD(info), connHasNoEnc, credLFixed, true)) {
							...
							System.out.println("Conn Vul at line: " + info.getLno() + " was patched with No errors!");
							// ======================== %4040% Patch is Found
							//
...
//				 String vulApp , String vsPath, String sslicesPath,  Info info,  AppMD amd, boolean print
					...
					if (PatternGen.findPatternsAndApply_usingExistingPatches(v_appSrc, v_slicePath,
						...
						System.out.println("Vul at line: " + info.getLno() + " WASS patched with No errors!");
						// ======================== %4040% Patch is Found
						//
						// TODO: create collect_GPatches_stat();
						// boolean print, boolean is_ps, boolean slicerfailed
...
//				}
	...
	public static void start_dcafixer(String projectName, String projectSrc, String projectJar, String resultCsv) throws Exception {
		String CsvDir = G.CsvDir;
		long start = System.currentTimeMillis();
		// ----- GPatches stats file header
		StrUtil.write_tofile("out/dcafixer-report/tmp.csv", //G.GPatchesTmpPath,
		...
		String csvDir = G.CsvDir;
		// project name to name the report , project src path and jar path
//		String paperExSrc = G.projectsPath  + "smallBank/src/TSet2/";
//		String paperExJar = G.projectsJarsPath + "smallBank.jar";
		if(resultCsv == null || resultCsv.length()== 0 ) {
			//Default path to store cvs
			resultCsv = csvDir + projectName+".csv";
		}

//		String paperExName = "PaperExample_vul";
		PrintStream o100 = new PrintStream(new File(G.ResultsPath +"100-"+ projectName + ".txt"));
		System.setOut(o100);
		...
		for (String f : LocatingPIs.SCList) {
			FixerNExSet.fault_localization(3, appCGPEx, projectJar, f, projectName, true);
		}
...
//		/Users/dareen/Fixer/DCAFixer_Experimets/Results
		// G.ResultsPath;
//        PrintStream o = new PrintStream(new File("/Users/dareen/Fixer/DCAFixer_Experimets/mariadb_ouput.txt"));
//        System.setOut(o);

//		String CsvDir = "/Users/dareen/Fixer/DCAFixer_Experimets/Csv_files/";//
		String CsvDir = G.CsvDir;
		int N = 0, P = 0;
		long start = System.currentTimeMillis();
		// ----- GPatches stats file header
...
//		// /Users/dareen/Fixer/DCAFixer_Experimets/SQLIFix-projects/jar_files/banking-app.jar
//		String proj1Csv = CsvDir + "banking-app.csv";
//		String proj1Name = "banking-app";
//		PrintStream o1 = new PrintStream(new File(G.ResultsPath + "01-" + proj1Name + ".txt"));
//		System.setOut(o1);
//
...
//		N = 11;
//
//		LocatingPIs.analyze_project(proj1Jar, proj1Src, proj1Csv, false, P, N);
//		System.out.println("Start building the CG .......");
...
////			FixerNExSet.fault_localization(2,appCG, G.CG_AllEPoints_MyExGUI,proj1Jar, f, proj1Name, false);//CG_AllEPoints
//			FixerNExSet.fault_localization(2, appCG1, proj1Jar, f, proj1Name, false);
//		}
//		print_all_results(proj1Name);//
//		// TODO: - - - Static Testing
//		// 1- Jar, get new jar path, same source code.
//		// 2- Repeat the steps
//		// 3- compute the difference. Fixed = (vul1 * 2) - vul2
...
//		N = 0;
//
//		PrintStream o2 = new PrintStream(new File(G.ResultsPath +"02-"+ proj2Name + ".txt"));
//		System.setOut(o2);
//
//		System.out.println("Start building the CG .......");
...
//		System.out.println("Done building the CG .......");
//
//		LocatingPIs.analyze_project(proj2Jar, proj2Src, proj2Csv, false, P, N);
//		for (String f : LocatingPIs.SCList) {
//			// FixerNExSet.fault_localization(1, G.CG_MainEPoints_MyEx,proj2Jar, f,
//			// proj2Name, false);
//			FixerNExSet.fault_localization(1, appCG2, proj2Jar, f, proj2Name, true);
//		}
...
//
//		PrintStream o3 = new PrintStream(new File(G.ResultsPath +"03-"+ proj3Name + ".txt"));
//		System.setOut(o3);
//
//		LocatingPIs.analyze_project(proj3Jar, proj3Src, proj3Csv, false, P, N);
//
//		System.out.println("Start building the CG .......");
...
//
//		for (String f : LocatingPIs.SCList) {
//			FixerNExSet.fault_localization(1, appCG3, proj3Jar, f, proj3Name, false);
//			// FixerNExSet.fault_localization(3, G.CG_AllEPoints,proj3Jar, f, proj3Name,
//			// false);
...
//
//		PrintStream o4 = new PrintStream(new File(G.ResultsPath + "04-"+proj4Name + ".txt"));
//		System.setOut(o4);
//
//		System.out.println("Start building the CG .......");
...
//		System.out.println("Done building the CG .......");
//
//		LocatingPIs.analyze_project(proj4Jar, proj4Src, proj4Csv, false, P, N);
//
//		for (String f : LocatingPIs.SCList) {
//			FixerNExSet.fault_localization(3, appCG4, proj4Jar, f, proj4Name, false);
//			// FixerNExSet.fault_localization(1,G.CG_AllEPoints,proj4Jar, f, proj4Name,
//			// false);
//		}
...
//
//		PrintStream o5 = new PrintStream(new File(G.ResultsPath +"05-"+ proj5Name + ".txt"));
//		System.setOut(o5);
//
//		System.out.println("Start building the CG .......");
...
//		System.out.println("Done building the CG .......");
//
//		LocatingPIs.analyze_project(proj5Jar, proj5Src, proj5Csv, false, P, N);
//
//		for (String f : LocatingPIs.SCList) {
//			FixerNExSet.fault_localization(3, appCG5, proj5Jar, f, proj5Name, false);
////			FixerNExSet.fault_localization(3,G.CG_AllEPoints,proj5Jar, f, proj5Name, false);
//		}
...
//
//		PrintStream o6 = new PrintStream(new File(G.ResultsPath + "06-"+proj6Name + ".txt"));
//		System.setOut(o6);
//
//		LocatingPIs.analyze_project(proj6Jar, proj6Src, proj6Csv, false, P, N);
//		System.out.println("Start building the CG .......");
...
//		System.out.println("Done building the CG .......");
//		for (String f : LocatingPIs.SCList) {
//			FixerNExSet.fault_localization(3, appCG6, proj6Jar, f, proj6Name, false);
//		}
...
//
//		PrintStream o7 = new PrintStream(new File(G.ResultsPath +"07-"+ proj7Name + ".txt"));
//		System.setOut(o7);
//
//		LocatingPIs.analyze_project(proj7Jar, proj7Src, proj7Csv, false, P, N);
//		System.out.println("Start building the CG .......");
...
//		System.out.println("Done building the CG .......");
//		for (String f : LocatingPIs.SCList) {
//			FixerNExSet.fault_localization(3, appCG7, proj7Jar, f, proj7Name, true);
//		}
...
//
//		PrintStream o8 = new PrintStream(new File(G.ResultsPath +"08-"+ proj8Name + ".txt"));
//		System.setOut(o8);
//
//		LocatingPIs.analyze_project(proj8Jar, proj8Src, proj8Csv, false, P, N);
//		System.out.println("Start building the CG .......");
//		CGClass appCG8 = EBCreation.BuildCG(proj8Jar, G.CG_AllEPoints_MyEx);
//		System.out.println(CallGraphStats.getStats(appCG8.getCg()));
//		System.out.println("Done building the CG .......");
//		for (String f : LocatingPIs.SCList) {
//			FixerNExSet.fault_localization(3, appCG8, proj8Jar, f, proj8Name, true);
//		}
...
//
//		PrintStream o9 = new PrintStream(new File(G.ResultsPath +"09-"+ proj9Name + ".txt"));
//		System.setOut(o9);
//
//		LocatingPIs.analyze_project(proj9Jar, proj9Src, proj9Csv, false, P, N);
//		System.out.println("Start building the CG .......");
...
//		System.out.println("Done building the CG .......");
//		for (String f : LocatingPIs.SCList) {
//			FixerNExSet.fault_localization(4, appCG9, proj9Jar, f, proj9Name, true);
//		}
...
//		PrintStream o10 = new PrintStream(new File(G.ResultsPath +"10-"+ proj10Name + ".txt"));
//		System.setOut(o10);
//		LocatingPIs.analyze_project(proj10Jar, proj10Src, proj10Csv, false, P, N);
//
//		System.out.println("Start building the CG .......");
...
//
//		for (String f : LocatingPIs.SCList) {
//			FixerNExSet.fault_localization(3, appCG10, proj10Jar, f, proj10Name, true);
//		}
...
//
//		PrintStream o11 = new PrintStream(new File(G.ResultsPath +"11-"+ proj11Name + ".txt"));
//		System.setOut(o11);
//
//		LocatingPIs.analyze_project(proj11Jar, proj11Src, proj11Csv, false, P, N);
//		System.out.println("Start building the CG .......");
...
//
//		for (String f : LocatingPIs.SCList) {
//			FixerNExSet.fault_localization(3, appCG11, proj11Jar, f, proj11Name, true);
//		}
//
...
//
//		PrintStream o12 = new PrintStream(new File(G.ResultsPath + "12-"+ proj12Name + ".txt"));
//		System.setOut(o12);
//
//		LocatingPIs.analyze_project(proj12Jar, proj12Src, proj12Csv, false, P, N);
//		System.out.println("Start building the CG .......");
...
//		System.out.println("Done building the CG .......");
//		for (String f : LocatingPIs.SCList) {
//			FixerNExSet.fault_localization(3, appCG12, proj12Jar, f, proj12Name, true);
//		}
...
//
//		PrintStream o13 = new PrintStream(new File(G.ResultsPath +"13-"+ proj13Name + ".txt"));
//		System.setOut(o13);
//
//		LocatingPIs.analyze_project(proj13Jar, proj13Src, proj13Csv, false, P, N);
//		System.out.println("Start building the CG .......");
...
//		System.out.println("Done building the CG .......");
//		for (String f : LocatingPIs.SCList) {
//			FixerNExSet.fault_localization(1, appCG13, proj13Jar, f, proj13Name, true);
//		}
...
//
//		PrintStream o14 = new PrintStream(new File(G.ResultsPath +"14-"+ proj14Name + ".txt"));
//		System.setOut(o14);
//
//		LocatingPIs.analyze_project(proj14Jar, proj14Src, proj14Csv, false, P, N);
//		System.out.println("Start building the CG .......");
...
//		System.out.println("Done building the CG .......");
//		for (String f : LocatingPIs.SCList) {
//			FixerNExSet.fault_localization(2, appCG14, proj14Jar, f, proj14Name, true);
//		}
...
//
//		PrintStream o19 = new PrintStream(new File(G.ResultsPath +"15-"+ testProj01Name + ".txt"));
//		System.setOut(o19);
//
//		LocatingPIs.analyze_project(testProj01Jar, testProj01Src, testProj01Csv, false, P, N);
//
//		System.out.println("Start building the CG .......");
...
//
//		for (String f : LocatingPIs.SCList) {
//			FixerNExSet.fault_localization(3, testApp01CG, testProj01Jar, f, testProj01Name, true);
//		}
...
//
//		PrintStream o20 = new PrintStream(new File(G.ResultsPath +"16-"+ testProj02Name + ".txt"));
//		System.setOut(o20);
//
//		LocatingPIs.analyze_project(testProj02Jar, testProj02Src, testProj02Csv, false, P, N);
//
//		System.out.println("Start building the CG .......");
...
//
//		for (String f : LocatingPIs.SCList) {
//			FixerNExSet.fault_localization(3, testApp02CG, testProj02Jar, f, testProj02Name, true);
//		}
...
//
//		PrintStream o21 = new PrintStream(new File(G.ResultsPath +"17-"+ testProj03Name + ".txt"));
//		System.setOut(o21);
//
//		LocatingPIs.analyze_project(testProj03Jar, testProj03Src, testProj03Csv, false, P, N);
//
//		System.out.println("Start building the CG .......");
...
//
//		for (String f : LocatingPIs.SCList) {
//			FixerNExSet.fault_localization(3, testApp03CG, testProj03Jar, f, testProj03Name, true);
//		}
...
//
//		PrintStream o22 = new PrintStream(new File(G.ResultsPath +"18-"+ testProj04Name + ".txt"));
//		System.setOut(o22);
//
//		LocatingPIs.analyze_project(testProj04Jar, testProj04Src, testProj04Csv, false, P, N);
//
//		System.out.println("Start building the CG .......");
...
//
//		for (String f : LocatingPIs.SCList) {
//			FixerNExSet.fault_localization(3, testApp04CG, testProj04Jar, f, testProj04Name, true);
//		}
...
//		N = 0;
//
//		PrintStream o23 = new PrintStream(new File(G.ResultsPath +"19-"+ testProj05Name + ".txt"));
//		System.setOut(o23);
//
//		LocatingPIs.analyze_project(testProj05Jar, testProj05Src, testProj05Csv, true, P, N);
//		System.out.println(LocatingPIs.getSCList().toString());
//		System.out.println("Start building the CG ............");
...
//
//		for (String f : LocatingPIs.SCList) {
//			FixerNExSet.fault_localization(3, testApp05CG, testProj05Jar, f, testProj05Name, true);
//		}
...
//
//		PrintStream o24 = new PrintStream(new File(G.ResultsPath +"20-"+ testProj06Name + ".txt"));
//		System.setOut(o24);
//
//		LocatingPIs.analyze_project(testProj06Jar, testProj06Src, testProj06Csv, false, P, N);
//
//		System.out.println("Start building the CG .......");
...
//
//		for (String f : LocatingPIs.SCList) {
//			FixerNExSet.fault_localization(4, testApp06CG, testProj06Jar, f, testProj06Name, true);
//		}
...
//
//		PrintStream o25 = new PrintStream(new File(G.ResultsPath +"21-"+ testProj07Name + ".txt"));
//		System.setOut(o25);
//		P=0; N=0;
//		LocatingPIs.analyze_project(testProj07Jar, testProj07Src, testProj07Csv, false, P, N);
//
//		System.out.println("Start building the CG .......");
////				System.out.println("CG from SrcDir ..");
...
//
//		for (String f : LocatingPIs.SCList) {
//			FixerNExSet.fault_localization(3, testApp07CG, testProj07Jar, f, testProj07Name, true);
//		}
...
//
//		String projCsv;
//		String proj01Src = G.projectsPath + "DCAFixer_bookstore2/src";
//		String proj01Jar = G.projectsPath + "DCAFixer_bookstore2/dist/DCAFixer_bookstore2.jar";
//		projCsv = CsvDir + "DCA_bookstore.csv";
//		P = 1;
//		N = 5;
//
//		String proj01Name = "DCA_bookstore";
//		PrintStream o15 = new PrintStream(new File(G.ResultsPath + "22-"+ proj01Name + ".txt"));
//		System.setOut(o15);
//
//		LocatingPIs.analyze_project(proj01Jar, proj01Src, projCsv, false, P, N);
//		System.out.println("Start building the CG .......");
...
//
//		for (String f : LocatingPIs.SCList) {
//			FixerNExSet.fault_localization(3, appCG01, proj01Jar, f, proj01Name, true);
//		}
//		print_all_results(proj01Name);
//
//		// === Done , all worked ---------------------- DCA_NewsPaper ----------------
//		String proj02Src = G.projectsPath + "DCAFixer_NewsPaper/src/dcafixer_newspaper";
//		String proj02Jar = G.projectsPath + "DCAFixer_NewsPaper/dist/DCAFixer_NewsPaper.jar";
//		String proj02Csv = CsvDir + "DCA_NewsPaper.csv";
//		P = 5;
//		N = 9;
//
//		String proj02Name = "DCA_NewsPaper";
//		PrintStream o16 = new PrintStream(new File(G.ResultsPath +"23-"+ proj02Name + ".txt"));
//		System.setOut(o16);
//
//		LocatingPIs.analyze_project(proj02Jar, proj02Src, proj02Csv, false, P, N);
//		System.out.println("Start building the CG .......");
...
//
//		for (String f : LocatingPIs.SCList) {
//			FixerNExSet.fault_localization(3, appCG02, proj02Jar, f, proj02Name, true);
//		}
//		print_all_results(proj02Name);
//
//		// ==== Done, all cases succeed with makeRTABuilder ----------------------
//		// DCA_FuelOrders ----------------
//		String proj03Src = G.projectsPath + "DCAFixer_FuelOrdersClient/src/fuelordersclient";
//		String proj03Jar = G.projectsPath + "DCAFixer_FuelOrdersClient/dist/DCAFixer_FuelOrdersClient.jar";
//		String proj03Csv = CsvDir + "DCA_FuelOrders.csv";
//		P = 5;
//		N = 2;
//
//		String proj03Name = "DCA_FuelOrders";
//		PrintStream o17 = new PrintStream(new File(G.ResultsPath +"24-"+ proj03Name + ".txt"));
//		System.setOut(o17);
//
//		LocatingPIs.analyze_project(proj03Jar, proj03Src, proj03Csv, false, P, N);
//		System.out.println("Start building the CG .......");
////			 	System.out.println("CG from SrcDir ..");
////			 	///Users/dareen/Fixer/DCAFixer_Experimets/Github_Dataset/Used_in_Fixer/FuelOrdersClient-master/src/main/java/fuelordersclient/PetrolInsertForm.java
...
//
//		// Done ---------------------- DCA_hospital ----------------
//		String proj04Src = G.projectsPath  + "DCAFixer_hospital-database-with-JDBC-client-master/JDBC_CLIENT/src";
//		//G.projectsPath  + "DCAFixer_hospital-database-with-JDBC-client-master/JDBC_CLIENT/src";
//		String proj04Jar = G.projectsPath  + "DCAFixer_hospital-database-with-JDBC-client-master/JDBC_CLIENT/dist/DCAFixer_Hospital.jar";
////		"/Users/dareen/Fixer/DCAFixer_Experimets/Github_Dataset/Used_in_Fixer/DCAFixer_hospital-database-with-JDBC-client-master/JDBC_CLIENT/dist/DCAFixer_Hospital.jar";
//		String proj04Csv = CsvDir + "DCA_hospital.csv";
//		P = 3;
//		N = 5;
//
//		String proj04Name = "DCA_hospital";
//		PrintStream o18 = new PrintStream(new File(G.ResultsPath +"25-"+ proj04Name + ".txt"));
//		System.setOut(o18);
//
//		LocatingPIs.analyze_project(proj04Jar, proj04Src, proj04Csv, false, P, N);
//
//		System.out.println("Start building the CG .......");
...
//
//		for (String f : LocatingPIs.SCList) {
//			FixerNExSet.fault_localization(3, appCG04, proj04Jar, f, proj04Name, true);
//		}
//		print_all_results(proj04Name);
//		// ****************************************************************************************************
//
//		// Dirs to clean from _fixed.java ==== > Now we only need to check
//		// "G.projectsPath"
//		// G.projectsPath +
//		// "DCAFixer_hospital-database-with-JDBC-client-master/JDBC_CLIENT/src";
//		// "/Users/dareen/Fixer/DCAFixer_Experimets/SQLIFix-projects/";
//		// "/Users/Dareen/NetBeansProjects/"; === > No need
//
//		// ********************************************* PaperExample
//		// *******************************************************
//		String paperExSrc = G.projectsPath  + "smallBank/src/TSet2/";
...
//		N = 1;
//		String paperExName = "PaperExample_vul";
//		PrintStream o100 = new PrintStream(new File(G.ResultsPath +"100-"+ paperExName + ".txt"));
//		System.setOut(o100);
//		LocatingPIs.analyze_project(paperExJar, paperExSrc, paperExCsv, false, P, N);
//
//		System.out.println("Start building the CG .......");
//		CGClass appCGPEx = EBCreation.BuildCG(paperExJar, G.CG_AllEPoints_MyExGUI);
//		System.out.println(CallGraphStats.getStats(appCGPEx.getCg()));
//		System.out.println("Done building the CG .......");
//
//
//		for (String f : LocatingPIs.SCList) {
//			FixerNExSet.fault_localization(3, appCGPEx, paperExJar, f, paperExName, true);
//		}
//		print_all_results(paperExName);
//
//		// ********************************************* SySet experiment *********************************************
//		// !!!!! For this experiment I stopped fixing CONN problems please uncomment the continue comment around line 632 and 633
//		sySetFlag = true;
//		String sySetSrc = //"/Users/dareen/eclipse-workspace/SySetExp/src/cases"; //one case
//				"/Users/dareen/Fixer/DCAFixer_Experimets/SQLIFIX-main/Data_Set/Synthetic/Apps_Before";// all cases
//		String sySetJar =  //"/Users/dareen/Fixer/DCAFixer_Experimets/SQLIFix-projects/jar_files/sysetafterCases.jar";
//				"/Users/dareen/Fixer/Experiments/JARS/apps-before.jar";
//		String sySetCsv = CsvDir + "PaperExample_vul.csv";
//		P = 77;
//		N = 1;
//		String sySetName = "SySet";
//		PrintStream o200 = new PrintStream(new File(G.ResultsPath +"200-"+ sySetName + ".txt"));
//		System.setOut(o200);
//		LocatingPIs.analyze_project(sySetJar, sySetSrc, sySetCsv, false, P, N);
//
//		System.out.println("Start building the CG .......");
//		CGClass appCGPEx = EBCreation.BuildCG(sySetJar, G.CG_AllEPoints_MyExGUI);
//		System.out.println(CallGraphStats.getStats(appCGPEx.getCg()));
//		System.out.println("Done building the CG .......");
//
//
//		for (String f : LocatingPIs.SCList) {
//			FixerNExSet.fault_localization(3, appCGPEx, sySetJar, f, sySetName, true);
//		}
//		print_all_results(sySetName);
