...
//			System.out.println("Columns in select part:");
			for (SelectItem s : ColumnsList) {
				s.getASTNode();
				SimpleNode node = s.getASTNode();// CCJSqlParserUtil.parseAST(s);
...
//				System.out.println("col: " + col);
			...
			if (insertStatement.getItemsList() != null) {
				if (insertStatement.getItemsList() instanceof ExpressionList list) {
...
//					|| uiVar.contains(parts[0]) || uiVar.contains(parts[parts.length-2]) 
		...
		if(uiVar!=null){
            return uiVar.contains(parts[0]) || uiVar.contains(parts[parts.length - 2]);
			}
		}
		return false;
	}
	...
	public static void parse_query_dt(String query, boolean print, List<String> uiVar, String query_type) throws JSQLParserException, IOException {
//		if (print) System.out.println("----- In parse_query");
		clear_all_values(query);
		
		// ====================================
		if(query_type.equals("MethodCallExpr") || query_type.equals("parameter") || query.equals("parameter")) {
		...
		if(query_type.equals("StringLiteralExpr")) {
			prefered_sol = G.SEC;
			msg = "secure";
			List<Character> I = new ArrayList<>();
			I.add('_');
			qs_ql.setvalues('_', '_', I);
			return;
		}
...
//		Sig qs_ql = create_query_sig(lq);
		...
		/* fixing steps */ if (print)
			System.out.println("4# Query Sig from qs_ql:" + qs_ql);
...
//		Query_="\"insert into gz_second_house(`property`,`area`,`section`,`address`,`layout`,`price`,`average_price`,`contact`,`identity`,`phone`,`website`,`atime`) values(\" + \"'\" + resultItems.get(\"property\") + \"',\" + \"'\" + resultItems.get(\"area\") + \"',\" + \"'\" + resultItems.get(\"section\") + \"',\" + \"'\" + resultItems.get(\"address\") + \"',\" + \"'\" + resultItems.get(\"layout\") + \"',\" + \"'\" + resultItems.get(\"price\") + \"',\" + \"'\" + resultItems.get(\"average_price\") + \"',\" + \"'\" + resultItems.get(\"contact\") + \"',\" + \"'\" + resultItems.get(\"identity\") + \"',\" + \"'\" + resultItems.get(\"phone\") + \"',\" + \"'\" + resultItems.get(\"website\") + \"',\" + \"'\" + resultItems.get(\"atime\") + \"'\" + \")\"";
//		Query_="\"INSERT INTO job_file_table VALUES(\" + \"'\" + f.getName() + \"',\" + \"'\" + job.getName() + \"',\" + \"'\" + f.getAbsolutePath() + \"',\" + \"\" + job.getFileType(job.getFileIndex(f.getName())) + \",\" + \"\" + job.getFileStatus(job.getFileIndex(f.getName())) + \")\"";//1, but result is 2!
//		Query_="Integer.toString(bid) + \";\"";// result=2, but there is no solution
//		Query_="\"select * from customers where cus_id  = '\" +answer+\"'\";"; //1, Parser failed to clean or parse!!!!
//		Query_="\"DROP TABLE\" + tName";//2
//		Query_="\"select name,type from items where item_code = \" + item_code[i]";//1
...
////			Query_="select p.name as patient_name, p.surname as patient_surname, a.t as date from appointments a, doctors d, patients p where a.patientamka=p.patientamka and a.doctoramka=d.doctoramka and a.t>(select date(max(t))from appointments) - 30 and a.t>current_date-30 and d.doctoramka='amka'";
//			Query_ = "select p.name as patient_name, p.surname as patient_surname, a.t as date\n" +
//	                "from appointments a, doctors d, patients p\n" +
//	                "where a.patientamka=p.patientamka and a.doctoramka=d.doctoramka\n" +
//	                "and a.t>(select date(max(t))from appointments) - 30 \n" +
//	                "and a.t>current_date-30\n" +
//	                "and d.doctoramka='\"+amka+\"'";
//			Query_ = "SELECT * FROM LOCATIONS WHERE COUNTRY_ID = ANY( SELECT COUNTRY_ID FROM COUNTRIES WHERE COUNTRY_NAME LIKE \" +id1+ \");";//1
		Query_="\"select \" + ThermodynamicMetrics.STEP + \" step, \" + ThermodynamicMetrics.NUM_OF_STEPS + \" num_of_steps, \" + ThermodynamicMetrics.DETAIL_GROUP + \" detail_group, \" + \"id \" + \" from \" + tableName + \" where id in (\" + idValues.toString() + \")\"";
			QParser.parse_query(Query_, true,null);
			String clean_q2 = ReplaceColumnValues.cleanStatement(Query_);
