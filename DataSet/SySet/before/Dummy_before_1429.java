...
public class Dummy_before_1429 {
	...
	public static int sendRequest(Connection conn, String rand1 ,String val1 ) throws SQLException {
		...
		ResultSet rs = stmt.executeQuery("SELECT DEPARTMENT_ID, MANAGER_ID, LOCATION_ID FROM DEPARTMENTS WHERE MANAGER_ID >" + val1+" OR LOCATION_ID >" + rand1);
		int rows = 0;
		if (rs != null) {
			while (rs.next()) {
				rows++;
