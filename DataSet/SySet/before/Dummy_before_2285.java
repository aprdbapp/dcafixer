...
public class Dummy_before_2285 {
	...
	public static int sendRequest(Connection conn, String rand8 ,String val2 ) throws SQLException {
		...
		ResultSet rs = stmt.executeQuery("SELECT EMPLOYEE_ID, MANAGER_ID, DEPARTMENT_ID FROM EMPLOYEES WHERE MANAGER_ID <>" + val2+" AND DEPARTMENT_ID <" + rand8);
		int rows = 0;
		if (rs != null) {
			while (rs.next()) {
				rows++;
