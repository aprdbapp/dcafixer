...
public class Dummy_before_1369 {
	...
	public static int sendRequest(Connection conn, String rand0 ,String var2 ) throws SQLException {
		...
		ResultSet rs = stmt.executeQuery("SELECT DEPARTMENT_ID, DEPARTMENT_NAME, MANAGER_ID FROM DEPARTMENTS WHERE DEPARTMENT_NAME >=" + var2+" AND MANAGER_ID >" + rand0);
		int rows = 0;
		if (rs != null) {
			while (rs.next()) {
				rows++;
