...
public class Dummy_before_1316 {
	...
	public static int sendRequest(Connection conn, String rand0 ,String var2 ) throws SQLException {
		...
		ResultSet rs = stmt.executeQuery("SELECT COUNTRY_ID, COUNTRY_NAME, REGION_ID FROM COUNTRIES WHERE COUNTRY_NAME <>" + var2+" AND REGION_ID <" + rand0);
		int rows = 0;
		if (rs != null) {
			while (rs.next()) {
				rows++;
