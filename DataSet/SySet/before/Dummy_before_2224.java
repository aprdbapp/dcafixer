...
public class Dummy_before_2224 {
	...
	public static int sendRequest(Connection conn, String rand8 ,String var2 ) throws SQLException {
		...
		ResultSet rs = stmt.executeQuery("SELECT EMPLOYEE_ID, MANAGER_ID, DEPARTMENT_ID FROM EMPLOYEES WHERE MANAGER_ID =" + var2+" OR DEPARTMENT_ID =" + rand8);
		int rows = 0;
		if (rs != null) {
			while (rs.next()) {
				rows++;
