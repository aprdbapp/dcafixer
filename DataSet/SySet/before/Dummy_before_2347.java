...
public class Dummy_before_2347 {
	...
	public static int sendRequest(Connection conn, String rand0 ,String var2 ) throws SQLException {
		...
		ResultSet rs = stmt.executeQuery("SELECT EMPLOYEE_ID, START_DATE, END_DATE FROM JOB_HISTORY WHERE START_DATE >=" + var2+" AND END_DATE >" + rand0);
		int rows = 0;
		if (rs != null) {
			while (rs.next()) {
				rows++;
