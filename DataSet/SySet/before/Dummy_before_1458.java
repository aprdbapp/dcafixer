...
public class Dummy_before_1458 {
	...
	public static int sendRequest(Connection conn, String rand1 ,String var2 ) throws SQLException {
		...
		ResultSet rs = stmt.executeQuery("SELECT DEPARTMENT_ID, MANAGER_ID, LOCATION_ID FROM DEPARTMENTS WHERE MANAGER_ID >=" + var2+" AND LOCATION_ID >" + rand1);
		int rows = 0;
		if (rs != null) {
			while (rs.next()) {
				rows++;
