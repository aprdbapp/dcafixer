...
public class Dummy_before_2039 {
	...
	public static int sendRequest(Connection conn, String rand6 ,String val2 ) throws SQLException {
		...
		ResultSet rs = stmt.executeQuery("SELECT EMPLOYEE_ID, SALARY, COMMISSION_PCT FROM EMPLOYEES WHERE SALARY =" + val2+" OR COMMISSION_PCT =" + rand6);
		int rows = 0;
		if (rs != null) {
			while (rs.next()) {
				rows++;
