...
public class Dummy_before_2518 {
	...
	public static int sendRequest(Connection conn, String rand2 ,String val2 ) throws SQLException {
		...
		ResultSet rs = stmt.executeQuery("SELECT EMPLOYEE_ID, JOB_ID, DEPARTMENT_ID FROM JOB_HISTORY WHERE JOB_ID >=" + val2+" OR DEPARTMENT_ID >" + rand2);
		int rows = 0;
		if (rs != null) {
			while (rs.next()) {
				rows++;
