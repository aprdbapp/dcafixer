...
public class Dummy_before_1812 {
	...
	public static int sendRequest(Connection conn, String rand3 ,String var1 ) throws SQLException {
		...
		ResultSet rs = stmt.executeQuery("SELECT EMPLOYEE_ID, PHONE_NUMBER, HIRE_DATE FROM EMPLOYEES WHERE PHONE_NUMBER >=" + var1+" OR HIRE_DATE >" + rand3);
		int rows = 0;
		if (rs != null) {
			while (rs.next()) {
				rows++;
