...
public class Dummy_before_1700 {
	...
	public static int sendRequest(Connection conn, String rand2 ,String val2 ) throws SQLException {
		...
		ResultSet rs = stmt.executeQuery("SELECT EMPLOYEE_ID, EMAIL, PHONE_NUMBER FROM EMPLOYEES WHERE EMAIL >" + val2+" OR PHONE_NUMBER >" + rand2);
		int rows = 0;
		if (rs != null) {
			while (rs.next()) {
				rows++;
