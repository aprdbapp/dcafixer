...
public class Dummy_before_2176 {
	...
	public static int sendRequest(Connection conn, String rand7 ,String val1 ) throws SQLException {
		...
		ResultSet rs = stmt.executeQuery("SELECT EMPLOYEE_ID, COMMISSION_PCT, MANAGER_ID FROM EMPLOYEES WHERE COMMISSION_PCT <=" + val1+" OR MANAGER_ID <" + rand7);
		int rows = 0;
		if (rs != null) {
			while (rs.next()) {
				rows++;
