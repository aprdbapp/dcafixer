...
public class Dummy_before_1232 {
	...
	public static int sendRequest(Connection conn, String rand0 ,String val1 ) throws SQLException {
		...
		ResultSet rs = stmt.executeQuery("SELECT COUNTRY_ID, COUNTRY_NAME, REGION_ID FROM COUNTRIES WHERE COUNTRY_NAME =" + val1+" AND REGION_ID =" + rand0);
		int rows = 0;
		if (rs != null) {
			while (rs.next()) {
				rows++;
