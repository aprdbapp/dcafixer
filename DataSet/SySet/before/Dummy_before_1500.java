...
public class Dummy_before_1500 {
	...
	public static int sendRequest(Connection conn, String rand0 ,String val1 ) throws SQLException {
		...
		ResultSet rs = stmt.executeQuery("SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME FROM EMPLOYEES WHERE FIRST_NAME =" + val1+" OR LAST_NAME =" + rand0);
		int rows = 0;
		if (rs != null) {
			while (rs.next()) {
				rows++;
