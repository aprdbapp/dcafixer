...
public class Dummy_before_1339 {
	...
	public static int sendRequest(Connection conn, String rand0 ,String val1 ) throws SQLException {
		...
		ResultSet rs = stmt.executeQuery("SELECT DEPARTMENT_ID, DEPARTMENT_NAME, MANAGER_ID FROM DEPARTMENTS WHERE DEPARTMENT_NAME >" + val1+" AND MANAGER_ID >" + rand0);
		int rows = 0;
		if (rs != null) {
			while (rs.next()) {
				rows++;
