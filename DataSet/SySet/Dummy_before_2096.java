...
public class Dummy_before_2096 {
	...
	public static int sendRequest(Connection conn, String rand6 ,String var1 ) throws SQLException {
		...
		ResultSet rs = stmt.executeQuery("SELECT EMPLOYEE_ID, SALARY, COMMISSION_PCT FROM EMPLOYEES WHERE SALARY <=" + var1+" OR COMMISSION_PCT <" + rand6);
		int rows = 0;
		if (rs != null) {
			while (rs.next()) {
				rows++;
